...
const compraAsiento = async (req, res) => {
    ...
    try {
        ...
        if(fecha < data.fechaActual){
            ...
            let infoasiento = await instance.AsientoExist(data)
            ...
            if (validAsiento.status == 200) idAsiento = parseInt(infoasiento[0]._id)
            let disponibilidad = await instance.getSeatAvailability(funcion)
            const asientoEncontrado = instance.AsientoEncontrado(disponibilidad, idAsiento)
            if (asientoEncontrado) {
                ...
                let resultado = (result.acknowledged) ? AsientoDto.templateBoleta(result): AsientoDto.templateNoBoleta()
